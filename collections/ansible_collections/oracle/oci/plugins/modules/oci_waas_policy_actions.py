...
DOCUMENTATION = """
---
module: oci_waas_policy_actions
description: >-
    Due to size constraints we have not included the documentation in the module. Please check L(here,
    https://docs.oracle.com/en-us/iaas/tools/oci-ansible-collection/latest/collections/oracle/oci/oci_waas_policy_actions_module.html)
...
class WaasPolicyActionsHelperGen(OCIActionsHelperBase):
    """
    Supported actions:
    ...
    def accept_recommendations(self):
        return oci_wait_utils.call_and_wait(
            call_fn=self.client.accept_recommendations,
            call_fn_args=(),
            call_fn_kwargs=dict(
                waas_policy_id=self.module.params.get("waas_policy_id"),
                protection_rule_keys=self.module.params.get("protection_rule_keys"),
            ),
    ...
    def purge_cache(self):
        action_details = oci_common_utils.convert_input_data_to_model_class(
            self.module.params, PurgeCache
        )
        return oci_wait_utils.call_and_wait(
            call_fn=self.client.purge_cache,
            call_fn_args=(),
            call_fn_kwargs=dict(
                waas_policy_id=self.module.params.get("waas_policy_id"),
                purge_cache=action_details,
            ),
            waiter_type=oci_wait_utils.WORK_REQUEST_WAITER_KEY,
            operation="{0}_{1}".format(
                self.module.params.get("action").upper(),
...
WaasPolicyActionsHelperCustom = get_custom_class("WaasPolicyActionsHelperCustom")


class ResourceHelper(WaasPolicyActionsHelperCustom, WaasPolicyActionsHelperGen):
    pass
